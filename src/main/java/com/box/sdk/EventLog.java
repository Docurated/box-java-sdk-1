...
public class EventLog implements Iterable<BoxEvent> {
    ...
    EventLog(BoxAPIConnection api, JsonObject json, long streamPosition, int limit) {
        ...
        if (nextStreamPosition.isString()) {
            this.nextStreamPosition = Long.valueOf(nextStreamPosition.asString());
        } else {
            this.nextStreamPosition = nextStreamPosition.asLong();
        }
    ...
    public static EventLog getEnterpriseEvents(BoxAPIConnection api, Date after, Date before, BoxEvent.Type... types) {
        return getEnterpriseEvents(api, STREAM_POSITION_NOW, after, before, ENTERPRISE_LIMIT, types);
    }
    ...
    private static EventLog getEnterpriseEventsForStreamType(
        ...
        JsonObject responseJSON = Json.parse(response.getJSON()).asObject();
        EventLog log = new EventLog(api, responseJSON, Long.valueOf(request.getPosition()), request.getLimit());
